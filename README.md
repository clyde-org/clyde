## Clyde
Clyde is a high-performance peer-to-peer (P2P) data acceleration engine designed  designed for rapid, large-scale delivery across heterogeneous compute environments. Currently, it specialises in fast container image distribution across nodes in a clusters. We are expanding its capabilities to support general content delivery, including artificial intelligence model weights, datasets, and remote repository content (e.g., Git LFS, Hugging Face, pip, etc.). Clyde integrates with Apull to enable lazy loading of images.
